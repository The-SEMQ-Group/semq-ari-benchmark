"""How much the movement bounds actually say.

:mod:`ari.change_profile` reports that a coordinate moved and bounds how
far. Having a bound is not the same as knowing something: a lower bound
of zero rules nothing out, and a finite upper bound can be far too loose
to act on. This module reports the quality of the bounds separately from
their existence, so a reader can tell an informative interval from a
vacuous one.

Every rate here names its denominator. Two are in play and they answer
different questions:

* **over all coordinates** — how much of the vector the bounds resolve.
* **over changed coordinates** — how much the bounds say about the
  changes that were actually detected.

Neither is the right one in general, so both are reported and each
carries its own count. When nothing changed, the rates over changed
coordinates are ``None`` rather than zero: no coordinate was available
to be informative about, which is not the same as every coordinate being
uninformative.

Generic codec bounds and norm-constrained bounds are kept apart. The
generic bounds hold for any input; the norm-constrained ones hold only
under a declared assumption about vector norms and are reported with it
(see ``QuantRegions.bounded_displacement``). Mixing them would let an
assumption travel silently into a figure.

Fractions measured here are results for the probe and the intervention
they were measured on. They are not constants of the method, and the
recorded configuration travels with them.

Nothing here measures functional harm. A threshold on movement is a
threshold on the representation; whether crossing it matters downstream
...
class BoundQuality:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "lower_positive_rate_of_changed": _rate(self.n_lower_positive,
                                                    self.n_changed),
...
def summarize_bounds(displacement: Any, *, over_changed_only: bool = False,
    ...
    ``over_changed_only`` restricts the width quantiles to coordinates
    whose symbol changed. The counts always cover every coordinate, so
    the two denominators stay recoverable either way.
    """
    ...
    widths = (hi[width_mask] - lo[width_mask]) if width_mask.any() else np.array([])
...
class ThresholdResolution:
    ...
    smallest possible movement is over, and *unresolved* when the
    interval straddles it. An unbounded upper bound can never be
    definitely below.

    This is a threshold on the representation. It is not a statement
    about downstream behaviour.
    """
...
def cost_record(*, n_inputs: int, n_coordinates: int, n_bins: int,
    ...
    A byte count or a timing means nothing without the input count and
    the machine it was taken on, so both travel with the numbers.
    """
    return {
