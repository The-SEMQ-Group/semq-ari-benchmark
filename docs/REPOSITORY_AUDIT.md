...
In the audit's working copy, six numerical LaTeX tables are generated by
`docs/paper/latex/build_tables.py`; its `--check` mode and
`tests/test_paper_evidence.py` fail when generated output differs from the
saved sources. Neither file is tracked on `main` or on this branch; both exist
only in the working tree of the paper branch `ilona/sdk-only-probe-and-paper`
and are named "when present" in the release checklist. All three figures now
load their plotted numbers from JSON, including the previously inline SciFact
and prefix curves.
...
The older advisor review (`docs/paper/REVIEW.md`) was removed from the
repository on 2026-09-16; its findings are summarized in this audit. Its
