...
> **Status: exploratory.** This was written and labelled as a preregistration.
> It does not earn that label and has been relabelled.
>
> The grid below was genuinely fixed before any cell was scored, and that part
> stands. The decision rule was not: it named no threshold, referred to
> "the near-null conditions' real interventions", which is not a coherent set,
> and existed only as prose that a human applied by eye. A rule reading
> "a non-zero share" was satisfied by 66 coordinates out of 246,058.
>
> The rule is now implemented in [`decide.py`](decide.py) with a declared
> threshold, declared condition roles, and document-resampled intervals. That
> version was written **after** the first results were seen, so it is not
> preregistered either, and the study is exploratory throughout. A rule that
> earns the label belongs with the fresh episodes in SEM-48 and SEM-49.
>
> The original rule is kept at the bottom, unedited, as the record of what went
> wrong.
...
bound, and the interval's lower end must clear zero. A point estimate alone
does not qualify a cell.
